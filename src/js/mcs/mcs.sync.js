 ...
 * Oracle Mobile Cloud Service Sync JavaScript SDK, Release: 17.2.5.0, E76062-04
*/
...
function persistenceUtils($common, $options) {
   ...
   * @returns {*}
   */
  function setNestedProperty2(object, property, value, nestedQuery) {
      ...
      if (property.length == (idx + 1)) {
        ...
        // handle usual property
          ...
          if(nestedQuery){
            ...
            for(var idx in nestedItems){
              if(nestedItems.hasOwnProperty(idx) && nestedItems[idx][nestedQuery.key.name] === nestedQuery.value){
                _.deepExtend(nestedItems[idx], value);
                break;
              }
            }
          } else {
...
function persistenceDB(name, $options) {
  ...
  if($options.isCordova){
    if(typeof LokiCordovaFSAdapter === 'function'){
      ...
      console.warn('LokiCordovaFSAdapter is not installed');
    }
  }
...
function persistenceMCSHandler($options, $common, $utils) {
  ...
  var handleMcsGet = function (response) {
    ...
    if (_.isEmpty(persistentPath.params)) {
      ...
      for (var idx in result) {
        if(result.hasOwnProperty(idx)) {
          ...
          data.items.push(clearObject(result[idx]));
        }
      }
    ...
    return clearObject(collection.findOne(query));
    // var result = collection.findOne(query);
    // if (result) {
  ...
  function clearObject(object){
    if(object){
      ...
      delete object['$loki'];
      delete object[URI_KEY];
      delete object[ETAG_KEY];
      delete object.meta;
    }
    return object;
  }
   ...
   * @returns {*}
   */
  this.postSuccessOperations = function(obj){
...
function persistenceRequestHandler($options, $common, $utils) {
   ...
   * @returns {{obj: *, result: *}}
   */
  function createObjFromUrlParamsForExistingForPost(obj, persistentUrlObj, queryParams, payload, dbPayload) {
    ...
    if (Array.isArray(nestedProperty) && nestedProperty.length > 0) {
      var nestedQuery;
      if(dbPayload){
        var key = getKeyForCurrentObject(persistentUrlObj, queryParams);
        nestedQuery = {
          key: key,
          value: dbPayload[key.name]
        };
      }
      result = $utils.setNestedProperty2(obj, nestedProperty, payload, nestedQuery);
    } else {
   ...
   * @returns {*}
   */
  function getKeyForCurrentObject(persistentUrlObj, queryParams){
  ...
  var handlePostRootObjectPayload = function (payload, isPersistentUrl, collection, force) {
    ...
    // let see if there is object with the same id
      ...
      payload[keyNameToCompare] = buildUniqueIDValue(isPersistentUrl, payload[keyNameToCompare]);
    }
  ...
  this.postSuccessOperations = function(obj){
    ...
    if (obj.syncObj.method === 'POST') {
      ...
      var parsed = $options.parseURL(cpObj.syncObj.url);
      var isPersistentUrl = $utils.isPersistUrl(parsed.path);
      ...
      if (!_.isEmpty(keyValueObj)) {
        ...
        // ok post used URL with key, so we have to create sub element here
        if (parent) {
          var result = createObjFromUrlParamsForExistingForPost(parent, isPersistentUrl, queryParams, response, beforeSyncPayload);
          var propertyName = isPersistentUrl.uri.tokens[isPersistentUrl.uri.tokens.length - 1].name;
          result.result[propertyName] = buildUniqueIDValue(isPersistentUrl, result.result[propertyName]);
          collection.update(result.obj);
          return _.cleanObject(result.result);
        }
        // parent object not in cache should add with child object
        else {
          var newObj = createObjFromUrlParamsForExistingForPost(keyValueObj, isPersistentUrl, queryParams, response);
          collection.insert(newObj.obj);
          return _.cleanObject(newObj.result);
        }
      }
      // deal with root items
        ...
        if (keyNameToCompare && !beforeSyncPayload.hasOwnProperty(keyNameToCompare)) {
          response = _.extendOwn(beforeSyncPayload, response);
          return _.cleanObject(collection.insert(response));
        } else {
          // find item by temporary id.
          var queryForObject = {};
          queryForObject[keyNameToCompare] = beforeSyncPayload[keyNameToCompare];
          var result = collection.findOne(queryForObject);
          _.extendOwn(result, response);
          return _.cleanObject(collection.update(result));
        }
      }
    }
...
function persistenceOracleRESTHandler($options, $common, $utils) {
  ...
  function buildOracleRESTResponse(obj) {
    if(_.isArray(obj)) {
      return {
        items: obj
      };
  ...
  var handleGet = function (response) {
    ...
    if (!_.isEmpty(keyValueObj)) {
      ...
      // deep search in the object!
      if (result) {
        var nestedObject= getNestedPropertyFromUrlParamsForExisting(result, isPersistentUrl, queryParams);
        return buildOracleRESTResponse(_.cleanObject(nestedObject));
      } else {
        // nothing inside, sorry!
        return null;
      }
  ...
  var handleGetStore = function (response) {
    ...
    // cache in between for easy access
    var payload = response.data.items || response.data;
    ...
    if (!_.isEmpty(keyValueObj)) {
      ...
      return buildOracleRESTResponse(newObj.result);
    } else {
  ...
  function syncXhr2(method, url, headers, data, responseType, timeout) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if(!headers['Oracle-Mobile-Client-SDK-Info']){
        // add header to all requests processed by sync express
        var infoHeader = $options.isCordova ? 'Cordova' : 'Web';
        infoHeader += ' ' + (typeof mcsVersion === 'undefined' ? 'Unknown' : mcsVersion);
      ...
      return new Promise(function (resolve) {
        ...
        if ($options.maxSyncAttempts === 0 || ((obj.$errorcounter || 0) <= $options.maxSyncAttempts)) {
          ...
          syncXhr2(cleanDbObject.method,
            ...
            .then($options.Module.postSuccessOperations.bind($options.Module))
            .catch(function (e) {
...
// syncXMLHttpRequest request initialization
  ...
  /**
   * This is to make _sync.operations.run availabe to manual synching, demo purpose only.
   */
  sync._sync = _sync;
  return sync;
}
